...
Career-Ops is a multi-agent job search system powered by Codex and built on Codex. It evaluates job listings, generates tailored ATS-optimized resumes, and manages your job search through an intelligent terminal interface.
...
│   ├── agents/
│   │   ├── evaluationAgent.js        # Codex agent for evaluation
│   │   ├── scanningAgent.js          # Codex agent for scanning
│   │   ├── resumeAgent.js            # Codex agent for resume gen
│   │   └── interviewPrepAgent.js     # Codex agent for prep guide
│   ├── templates/
...
CLAUDE_MODEL=Codex-3-5-sonnet-20241022  # Optional: Model to use
SCANNING_MODEL=Codex-3-5-haiku          # Optional: Faster model for scanning
```

### Settings (settings.json)
See `config/settings.json` for Codex customization:
- Model preferences
...
**Built with ❤️ using Codex & Codex**
